...
<div style="max-width:700px;margin:0 auto;">
    <div style="background:var(--warm-white);border-radius:8px;padding:2rem;border:1px solid var(--border);">
        <h1 style="font-family: 'Playfair Display', serif; font-size: 1.75rem; font-weight: 700; color: var(--brown-deep); margin: 0 0 1.5rem 0;">Edit Pengembalian</h1>

        <form action="{{ route('pengembalian.update', $pengembalian) }}" method="POST" style="display:flex;flex-direction:column;gap:1.25rem;">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="peminjaman_id" id="peminjaman_id" required style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;background:white;color:var(--ink);">
                    ...
                    @foreach($peminjamans as $peminj)
                        <option value="{{ $peminj->id }}" {{ $pengembalian->peminjaman_id == $peminj->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="date" name="tanggal_pengembalian" id="tanggal_pengembalian" value="{{ $pengembalian->tanggal_pengembalian->format('Y-m-d') }}" required style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="denda" id="denda" value="{{ $pengembalian->denda }}" style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;" placeholder="0">
                @error('denda')
                    <p style="color:#C41E3A;font-size:0.85rem;margin-top:0.25rem;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="catatan" id="catatan" rows="4" style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;resize:vertical;" placeholder="Masukkan catatan...">{{ $pengembalian->catatan }}</textarea>
                @error('catatan')
                    <p style="color:#C41E3A;font-size:0.85rem;margin-top:0.25rem;">{{ $message }}</p>
                @enderror
            </div>
