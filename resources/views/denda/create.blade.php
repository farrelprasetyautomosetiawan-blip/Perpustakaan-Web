...
<div style="max-width:700px;margin:0 auto;">
    <div style="background:var(--warm-white);border-radius:8px;padding:2rem;border:1px solid var(--border);">
        <div style="margin-bottom:1.5rem;">
            <h1 style="font-family: 'Playfair Display', serif; font-size: 1.75rem; font-weight: 700; color: var(--brown-deep); margin: 0 0 0.5rem 0;">Tambah Denda Baru</h1>
            <p style="color:var(--brown-mid);margin:0;font-size:0.95rem;">Buat data denda untuk pengembalian buku</p>
        </div>

        <form action="{{ route('denda.store') }}" method="POST" style="display:flex;flex-direction:column;gap:1.25rem;">
            @csrf
            ...
            <div>
                ...
                <input type="date" name="tanggal_pengembalian" id="tanggal_pengembalian" value="{{ old('tanggal_pengembalian') }}" required style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="denda" id="denda" value="{{ old('denda') }}" style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;" placeholder="Contoh: 5000">
                @error('denda')
                    <p style="color:#C41E3A;font-size:0.85rem;margin-top:0.25rem;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="catatan" id="catatan" rows="4" style="width:100%;padding:0.75rem;border:1px solid var(--border);border-radius:6px;font-size:0.95rem;font-family:'DM Sans',sans-serif;resize:vertical;" placeholder="Masukkan catatan...">{{ old('catatan') }}</textarea>
                @error('catatan')
                    <p style="color:#C41E3A;font-size:0.85rem;margin-top:0.25rem;">{{ $message }}</p>
                @enderror
            </div>
